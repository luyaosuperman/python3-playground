...
class Node(object):
    def __init__(self, value, count=0):
        ...
        self.right = None # more than

    def insertValue(self, value, noBump = False, count = 0, pre = 0):
        #print("tree insertValue", value, "current Node value", self.value, "count", self.count)
        if self.value == value:
            self.count += 1 + count
            if self.right != None:# and (not noBump):
                self.right.bumpCount()

        elif value < self.value:
            #go left
            
            if self.left != None:
                self.left.insertValue(value, noBump = True, pre=pre)
            else:
                self.left = Node(value, count = pre)


            self.count += 1 # we need to find count(?)<=value    
            if self.right != None:#  and (not noBump):
                self.right.bumpCount()
            ...
            if self.right != None:
                #if pre != 0:
                #    self.right.insertValue(value, pre=pre)
                #else:
                    self.right.insertValue(value, pre=self.count)
            else:
                #if pre != 0:
                #    self.right = Node(value ,count = pre)
                #else:
                    self.right = Node(value ,count = self.count)
    ...
    def getCount(self, value):
        #print("tree getCount", value, "current Node value", self.value, "count", self.count)
        if self.value == value:
            #print("return", self.count, "for get", value)
            return self.count
        elif value < self.value:
            if self.left != None:
                return self.left.getCount(value)
            else:
                return 0
        elif value > self.value:
            if self.right != None:
                return self.right.getCount(value)
            else:
                return self.count

    def dumpTree(self, outputDict):
        outputDict[self.value] = self.count
        if self.left != None:
            self.left.dumpTree(outputDict)
        if self.right != None:
            self.right.dumpTree(outputDict)
...
#class Solution(object):
    ...
    def getCount(self,num):
        #print("solution.getCount", num)
        if self.root is None:
            return 0
        else:
            return self.root.getCount(num)


    def putCount(self,num):
        #print("solution.putCount", num)
        if self.root is None:
            self.root = Node(num)
        else:
            #print(self.root)
            self.root.insertValue(num)
    ...
    def reversePairs(self, nums):
        ...
        for num in reversed(nums):
            ...
            result += count
            print("try to find count of target {0}, got {1}, result {2}".format(target, count, result))
            _count = self.getCount(num)
            self.putCount(num)
            print("updating freq of {0} from {1} to {2} ".format(num, _count, self.getCount(num) ) )

            #outputDictPre = outputDict
            outputDict = {}
            self.root.dumpTree(outputDict)
            print(outputDict)
            #assert(self.debug(outputDictPre,outputDict,num))

        print("result", result)
        return result

    def debug(self,outputDictPre,outputDict,num):
        for i in outputDict:
            if i < num:
                assert(outputDictPre[i] == outputDict[i])
            if i > num:
                assert(outputDictPre[i] + 1 == outputDict[i])
            if i == num:
                if i in outputDictPre:
                    assert(outputDictPre[i] + 1 == outputDict[i])
                else:
                    l = [ j for j in outputDictPre if j < i]
                    if len(l) > 0:
                        ii = max(l)
                        if ii != None:
                            print(ii,i,outputDict[ii],outputDict[i])
                            assert(1 + outputDict[ii] == outputDict[i])
                    else:
                        assert(1 == outputDict[i])
        return True
    ...
    def test_1(self):
        inputs = [1,3,2,3,1]
        outputs = 2
        #print(self.reversePairs(inputs))
        self.assertEqual(self.reversePairs(inputs), outputs)
...
if __name__ == '__main__':
    ...
    '''solution = Solution()
    inputs = [1,3,2,3,1]
    print(solution.reversePairs(inputs))'''