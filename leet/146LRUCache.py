...
class LRUCache(unittest.TestCase):
    ...
    def get(self, key):
        ...
        :rtype: int
        """
        #print("get", key, self.dict)
        if key in self.dict:
            #print("return", self.dict[key].value)
            node = self.dict[key]
            self.swimUp(node)
    ...
    def trace(self):
        ...
        while current is not None:
            print("trace", i, current.key, current.value, current, current.pre, current.next)
            if current.next is not None:
                if current.next.pre != current:
                    print(current.next.pre, current)
...
if __name__ == '__main__':
    ...
    for i in inputs:
        if len(i) == 1:
            print("__get {0} result {1}".format(i[0], lruCache.get(i[0])))
            #print("result:", lruCache.get(i[0]))
        if len(i) == 2:
