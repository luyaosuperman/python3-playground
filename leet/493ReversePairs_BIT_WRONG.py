...
#BIT cannot handle negative value
...
class Solution(unittest.TestCase):
    def reversePairs(self, nums):
        ...
        for num in reversed(nums):
            print("testing:", num)
            target = (num+1)//2 - 1 
            result += self.getSum(target)
            print("try to find count of target {0}, got {1}, result {2}".format(target, self.getSum(target), result))
            print("updating freq of {0} from {1} ".format(num, self.getSum(num)), end='')
            self.updateBit(num,  1)
            print("to {0}".format(self.getSum(num)))
            print(self.BIT)
        return result
    ...
    def updateBit(self,i,v):
        ...
        #print(" map to", i)
        #while i <= self.maxNum:
        while i <= self.maxNum + self.minNum + 1:
            #print("==>", i, self.BIT[i], self.BIT[i] + v)
            if i in self.BIT:
                self.BIT[i] += v
            else:
                self.BIT[i] = v
